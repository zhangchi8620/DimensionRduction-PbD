   ...
   {for (int run=0; run<10; run++)
     {cout << "Beginning run #" << run << endl;
        ...
        for (int episode_num=0; episode_num<100; episode_num++){ 
            cout << "episode_num: " << episode_num << "\t" << episode(10000) << endl;
				}
		 }
   ...
   {mcar_init();                                                   // initialize car's state
    ...
    do {step++;
        ...
        for (int i=0; i<N; i++) theta[i] += temp * e[i];           // update theta (learn)
    		...
    		cout << "\nstep:" << step<< "\tpos: " << mcar_position << ", vel:"<< mcar_velocity << endl;
    }
    while (!mcar_goal_p() && step<max_steps);                      // repeat until goal or time limit
    //cout << "pos: " << mcar_position << "\t vel:"<< mcar_velocity << endl;
...
#define mcar_min_position_x -0.3
#define mcar_max_position_x 0.3
#define mcar_min_position_y -0.2
#define mcar_max_position_y 0.2
#define mcar_min_position_z 0.3
#define mcar_max_position_z 0.4

#define mcar_max_velocity 0.005            // the negative of this is also the minimum velocity
#define mcar_goal_position 0.32

#define POS_WIDTH (0.02 / 8)               // the tile width for position
#define VEL_WIDTH (0.002 / 8)              // the tile width for velocity
...
void test(){
	  puts("Testing...");
	  //mcar_init();                      // initialize car state
    mcar_position = 0.3;
		mcar_velocity = 0.0;
		int step = 0;                                                  // now do a bunch of steps
    do {step++;
    ...
    while (!mcar_goal_p());                      // repeat until goal or time limit
    //cout << "pos: " << mcar_position << "\t vel:"<< mcar_velocity << endl;
